                    ...
                    <div class="card h-full">
                        <div class="card-header">
                            ...
                            <h5 class="card-title"><a href="{{url("admin/dashboard")}}" class="text-blue-500 mr-3"><i
                                        class="fa-solid fa-arrow-left-long"></i></a> Edit Calendar</h5>
                        </div>
                        <div class="card-body">
                            <form action="{{ $encLink }}" id="calendar_frm" method="post">
                                @csrf
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" id="calendar_name" name="calendar_name"
                                        placeholder="Enter Calendar Name" value="{{$calendarData->calendar_name}}">
                                </div>
                                <div class="form-group mb-3">
                                    ...
                                    <select id="category" name="category" class="form-select form-control"
                                        ...
                                        @foreach (Common::allCategories() as $item)
                                            <option value="{{$item->id}}" {{$calendarData->category_id==$item->id ? 'selected':''}}>{{$item->category_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group mb-3">
